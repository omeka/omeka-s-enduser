...
Used to extract the metadata generated by the Oral History Metadata Synchronizer Editor and contained in the xml file generated by that system. The OHMS extractor uses the JSON pointer to map the metadata. For more on working with OHMS in Omeka S, see the [OHMS Embed](ohmsembed.md) module. 
