...
The [DSpace Connector module](https://omeka.org/s/modules/DspaceConnector){target=_blank} allows you to connect an Omeka S instance to a [DSpace repository](https://duraspace.org/dspace/){target=_blank}. You can then import items and their associated information from that repository. The Omeka S item will maintain a connection back to the original item. This allows you to [refresh the information from the source when desired](#update-imported-resources).
...
### Collections
This tab will display the list of collections for the DSpace repository. The list is organized by the containing community. You can import either one collection at a time or the entire DSpace repository.

To import a single collection, click the "Import" button to the left of its name. This will automatically begin the import.

![The Collections tab showing some example collections imported from mars.gmu.edu's DSpace repository. This includes a grouping called "Administrative Collections" with collections "GMU Dark Archive", "Preservation Materials", and "Visual Arts". There is a button at the top of the screen to "Import entire repository." A button on each collection shows "Import collection". There are buttons to "Expand all" and "Collapse all" of the groupings.](../modules/modulesfiles/dspace_coll.png)

To import the entire repository, click "Import entire repository" at the top of the form.

!!! note
	Importing an entire DSpace repository with a more than 5,000 items is likely to flood the DSpace hosting server with requests. This will continue until failure. Consider importing collection by collection. If you still wish to import an entire large repository at once, the following might help:
	...
	* Consider temporarily inserting [a `sleep()` function](https://www.w3schools.com/php/func_misc_sleep.asp){target=_blank} between the import of each record in `Import.php`. This slows the process down slightly, but is not recommended for production.
...
The "Past DSpace Imports" page displays a table of past DSpace imports. It includes: 
- a checkbox option to **Undo**
- a checkbox option to **Re-run**
- the **Job ID** for the import
- the repository’s **Dspace Collection Link** (displayed as collection name if found)
...
DSpace items will import with any metadata fields recognized at the source. For example, `dc.title` or `dc.description.abstract`.  They will use `bibo:uri` to load the URI as given at the source item as `dc.identifier.uri`. This will be a clickable link that displays on public item pages. It allows users to see the item's source at its home DSpace repository. If the fields have one or more language tags, fields imported into Omeka will also use that language tag.
...
You can view past imports from the "Past DSpace Imports" page. There may be instances where you wish to undo a completed import and remove all associated items. to do this, check the box for each import you wish to undo and click the "Submit" button.
