...
The items and media are organized into an item set and then synced with a project in Scripto. Content made within Scripto can be imported into Omeka S as item and media metadata.
...
A Scripto **Project** allows you to organize the materials you want to work with. It also allows you to set how you intend to work with them. A project can either transcribe, translate, or describe; it cannot do more than one. 

The Omeka S Scripto module allows you to create multiple projects on a single Scripto installation. Like sites in Omeka S, Scripto projects can exist independently of each other. This means you can support a manuscript transcription project and a document translation project on the same installation.
...
Once you have installed the module and created your MediaWiki installation, go to the Modules tab of your Omeka S installation. Then, activate the Scripto module.
...
### Scripto Vocabulary
After you install and activate Scripto, it will add the Scripto [Vocabulary](../../content/vocabularies.md) to your Omeka S installation. This vocabulary contains the following properties:
...
Each Scripto project needs its own item set with which it can sync. Create the item set using items from your Omeka S installation. These should be the items you want to include in the project. You can add items to this item set later if you need to. 

## Scripto admin dashboard
The Scripto tab appears under the Modules section of the left-hand navigation on the Omeka S installation admin side. It takes you to the Scripto Dashboard. 
...
- A bar across the top. It includes your username and links to the dashboard, your contributions, your watchlist, and a button to Log Out of Scripto.
...
There is a dropdown in the upper right. Here, Omeka S global and site admins can browse all [Projects](scriptoproject.md), add a new Project, and browse all Scripto users. 
...
## Users
In addition to an Omeka S user account, any individual working on Scripto will need a MediaWiki user account too. They can sign up for this account from the public side of the Scripto interface: `<your omeka S url>/scripto/create-account`.

To change the status of transcriptions and sync projects to and from Omeka S, users must have [Bureaucrat level permissions](https://www.mediawiki.org/wiki/Manual:User_rights) on the MediaWiki installation. This *must be done on MediaWiki* and cannot be managed through the Scripto dashboard on Omeka S.

You may find it easiest to have administrators of Scripto projects use the same or similar user names on both installations. This helps keep track of who is doing what. 

### Browse users
From the "actions" dropdown in the upper right corner of the Scripto Dashboard, you can select "Browse users".

This will take you to a user browse page. This page shows all the Scripto users on this installation (from MediaWiki). 
...
To successfully uninstall Scripto, the module must be still be active. Do not deactivate Scripto prior to uninstalling. 
