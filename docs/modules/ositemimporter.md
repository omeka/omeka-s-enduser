...
- **Classes tab**: This view shows a list of the remote classes and whether they are on the local installation. Items will be assigned these classes only if they're on the local installation. If you find that there are classes available remotely that are not available locally, you will need to add them to your installation before proceeding to avoid data loss. Note that vocabularies can be added by installing and activating modules such as [Value Suggest](valuesuggest.md).
