...
The list below includes commonly requested options for configuration in the file `local.config.php`. This is located in the `/config` directory. 

For a full list of available configuration keys, see the [developer documentation page on configuration settings](https://omeka.org/s/docs/developer/configuration/){target=_blank}. 
...
Under `types`, set the maximum pixel dimensions for derivative images for media files. There are separate options for large, medium, and square. Defaults for these are 800, 200, and 200 pixels.

`thumbnailer_options` is an array of options passed to the specific thumbnailer in use. For example, the `imagemagick_dir` thumbnail option sets the path to the folder where ImageMagick's `convert` command can be found on the server. This is useful if Omeka S can't auto-detect the correct path for ImageMagick.

Use the [System Information page](admin-dashboard.md#system-information) to verify your installation's ImageMagick version. If there is an error when you click the button, you may need to set the configuration manually.

![The System Information buttons to retrieve the PHP path and the ImageMagick version.](files/systeminfo_buttons.png)

Set the thumbnailer to use under the `service_manager` key. Do so by setting the
...
You can also set the thumbnailer to `Omeka\File\Thumbnailer\NoThumbnail`. This will prevent your Omeka S installation from generating thumbnails. 
...
[Imagick and ImageMagick](https://www.imagemagick.org){target=_blank} are the same library. They can create thumbnails for more than 200 formats. Imagick is integrated into PHP and ImageMagick is the command-line version. 

ImageMagick may require you to manually set a path in `imagemagick_dir`. Imagick and GD do not require paths.
...
Omeka S uses background jobs for some long-running tasks. These might operate on many items or just otherwise might take a long time. Omeka S uses the PHP CLI (command-line interface) to run these jobs, the `php` command. An invalid PHP path can cause a number of problems for your Omeka installation. 
...
Use the [System Information page](admin-dashboard.md#system-information) to verify that your installation has identified the correct PHP path. If an error that occurs when you click the button might indicate you need to set the configuration manually.
...
You may also wish to manually select an earlier stable PHP version. A new version may be causing unexpected behavior in your Omeka site.
...
If you download the file to edit it, be sure to upload the changed version back to your Omeka installation.
...
The default is to use [sendmail](https://en.wikipedia.org/wiki/Sendmail){target=_blank}. This means the server is responsible for having mail delivery configured and set up. Sendmail generally requires no configuration on the Omeka S side. Another option for some servers is to configure a direct SMTP connection for sending mail. 
...
As of Omeka 4.2, garbage collection of sessions is now turned on by default. This is true even for servers that otherwise disable it.
