...
+ [Archival Resource Keys (ARKs)](https://arks.org){target=_blank}
+ DataCite, used to mint and manage [Digital Object Identifiers (DOIs)](https://datacite.org/create-dois/){target=_blank}
+ The [EZID service](https://ezid.cdlib.org/){target=_blank} to mint and manage [Archival Resource Keys (ARKs)](https://arks.org/){target=_blank} (for existing users only).
...
After installation, Persistent Identifiers should appear in the left-hand navigation under **Modules**. This link is available to users at the Global Admin and Supervisor levels. The "Settings" subpage features general settings, including:
...
!!! note
	Depending on your settings above, you should adjust your and other users' workflow: when creating an item, be sure to enter the existing PID before you save the new item. You may wish to keep "Assign PIDs to new items" turned off except in special circumstances. Remember that users lower than "Supervisor" will not have access to this setting and cannot turn it off, and leaving it on may have unintended consequences. 
...
Once generated, these locally-minted ARKs should immediately resolve to a stable, generic, site-agnostic landing page for its corresponding Omeka resource, complete with metadata and any media. Example: `http://n2t.net/ark:/99999/mk0081pk45`.
