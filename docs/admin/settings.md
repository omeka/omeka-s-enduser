...
Admin users may change the settings for the admin dashboard and general settings for all sites. To do this, use the **Settings** section on the left side of the admin dashboard (with the gear icon). 
...
**Time zone**: Sets the default time zone for the install. This will be used for error logging messages and to determine creation dates for items, item sets, and other materials on the install. This is a dropdown menu. 

**Locale**: A dropdown that lets you select from [available languages](https://www.transifex.com/omeka/omeka-s/){target=_blank} for the Omeka S core to change the language in which your installation's admin side displays.

**Enable version notifications**: Check this box to enable warning banners for new versions of Omeka S, themes, and modules. If unchecked, no user (at any role) will see update warnings, not even the Global Administrator. When checked, every applicable user can see them. For example, Supervisor users who can see the Modules page, Site Managers who can see the Themes pages, and every user on the `/admin` homepage.
...
**Results per page**: Changes the number of results (items, item sets, media) displayed per page. This applies to browsing in either public or admin sides of the installation.
...
**Default site**: This dropdown gives you control over where people land when they navigate to the base url of your install. You can either display a list of all sites in the install (default) or select a specific site from the dropdown. For the latter, this is where people will be redirected.

**Disable JSON-LD embed**: By default, Omeka embeds JSON-LD in resource browse and show pages. This is for the purpose of machine-readable metadata discovery. Check this to disable embedding.

![Display settings with fields as described](adminfiles/settings_display.png)

## Editing

**Set default item visibility to private**: Check this box to set the visibility to private for all new items made by all users. If the is box remains unchecked, all new items are public by default, unless set otherwise at the point of creation.
...
**Set default site visibility to private**: Check this box to set all new sites made by all users to private.

**Set default site page visibility to private**: Check this box to set the visibility of all new site pages made by all users to private.
...
**Media alt text property**: Select a field (for example, `dc:title` or `dc:description`) from the media description to use as alternative text. This is for when no alt text is explicitly set. [Alt text](https://webaim.org/techniques/alttext/){target=_blank} is used in lieu of images when a user is employing a screen reader and is an important part of meeting accessibility expectations. This also applies to thumbnails on browsing and search results pages. 
...
**Disable file validation**: a checkbox, unchecked by default. When unchecked, file uploads are limited by the following two fields. Check this box to allow any type of media or MIME type for upload.
...
The final two fields relate to setting up [Google reCAPTCHA](https://www.google.com/recaptcha/intro/index.html){target=_blank} for your site. You will need both a site key and a secret key. Enter them in the proper fields after signing up with reCAPTCHA:

**reCAPTCHA site key** enables display of the widget on your install's sites.

**reCAPTCHA secret key** allows your install and reCAPTCHA's server to communicate. 
