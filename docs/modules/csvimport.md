...
The [CSV Import module](https://omeka.org/s/modules/CSVImport){target=_blank} allows you to import items, item sets, media, and users into your Omeka S install. It accepts CSV (comma-separated values), TSV (tab-separated values), or ODS (OpenDocument Spreadsheet) files. This module is only available to [Global Administrator and Supervisor users](../admin/users.md).

CSV Import requires your Omeka S installation to [have PHP working in order to run background import jobs](../install.md#test-and-set-the-php-path). Before using CSV Import, you should confirm that PHP is recognized from the [System Information page](../admin-dashboard.md#system-information). 
...
!!! note
	CSV files for import **must be encoded in UTF-8**. When exporting or saving a new document, be sure to check that the encoding is UTF-8.

Most CSV Import options rely on you only importing one type of data. This might be a list of items, a list of item sets, a list of media, etc. There is the option for a [mixed-resource import](#mixed-resource-import), requiring one column that identifies the type of each row.

If the spreadsheet is already created, consider which columns you want to match to which [vocabulary properties](../content/vocabularies.md). Your CSV file **must have a header row** in order for the module to process it correctly. You may need to add a row at the top with column names.
...
There is a setting in the inital import settings to automap with simple labels. This will work with columns whose names match a vocabulary label without supplying the term. "Title" or "abstract" are good examples. Note that this option defaults to Dublin Core (`dcterms:title` and `dcterms:abstract`) before proceeding through other installed vocabularies.

If your column names are not exact the automapping feature will not recognize them. In this case, you should still label them something helpful so that you can manually map them while importing.

You might want to batch-import metadata or properties that come with a module (such as latitude and longitude from the Mapping module) or using structured vocabularies that come from modules (such as the data types from the Value Suggest module). If so, install and configure those modules first to ensure that the fields exist in your site's data model before trying to enter information into those fields. Data may be lost if you uninstall those modules later.

## Initial import settings

Start an import by clicking on the CSV Import tab on the left-hand navigation. This will open the initial "Import Settings" page. You can keep the defaults for most spreadsheets directly exported from a software program into the correct format.

- Use the "Choose File" button to select a spreadsheet from your computer.
- Choose one of the following options from the **CSV column delimiter** dropdown. This should match the formatting of your file, and is the character that separates different values in a row:
	- comma (default)
	- semi-colon
	- colon
	- tabulation
...
- From the **Import type** dropdown, select what you are importing:
	- Items
	- Item sets
	- Media (requires a column matching Media to already existing Items)
	- Mixed resources (The spreadsheet can include Item sets, Items, and Media. A column identifying the type of each row is required.)
	- Users.
...
![Mappings for a spreadsheet with ten columns. Some columns, such as those named Description and Title, automatically map to Dublin Core properties.](../modules/modulesfiles/csvimport_itemsMap1.png)
...
**Properties** allows you to select a property to map the column data to. Select the proerty from any of the installed vocabularies. Use the "Filter" field to search for a specific property.
...
**Item-specific data** has a dropdown to set an Item set by selected property. If you have a column identifying an Item set to which you want to add each item, you can set how it maps using this dropdown. Do this rather than putting all of the imported items into the same Item sets on the "Basic Settings" tab. You can either use the Item set's internal ID, or any one of its properties (such as title).
...
- **Resource template (by label)**: Set the template for an item by name. The name of the template entered in the spreadsheet must exactly match the name of the template in Omeka S.
...
**Media source** allows you to import [media](../content/media.md#add-media-to-an-item) along with your items. Select the sourcing method from the dropdown:
...
Be sure to click the "Apply Changes" at the bottom of the drawer or to keep your settings. 
...
You may have data in a column in your CSV that you do not want to import. If so, simply do not map that column to a property or data type.
...
- **Use multivalve separator**: Check this box to use the multivalue separator for data in this column. You set the multivalue separator in the initial import page. You can also change it in the Basic Settings tab.  
...
- **Data type**: A dropdown with at least three options. These options correspond to the [values](../content/items.md#values) one can use when adding properties to an item:
	...
	- Import as Omeka S resource. This will create [linked resources](../content/items.md#linked-resources). If you select this option, you must choose which property values to match to find the intended Omeka resource in your installation. Do this in the next **Resource identifier property** dropdown. This must be a unique property, so "Title" may not be a good choice. 
		- You can use the internal Omeka ID. A resource's ID is the number sequence at the end of the URL when on the view or edit page. For `/admin/item/11576` the ID is 11576. You can also see the resource's ID in the right-hand drawer on the resource's view page. Items, item sets, and media all have IDs.
		- You can include resources made in the same CSV as long as the linked resources were created in earlier rows and can be found with the unique property value indicated here. If you wish to do this, we recommend setting the batch number low (even to 1) on the Advanced Settings tab. This ensures resources are being fully created before another new resource tries to link to them. 
...
#### Batch edit
Select one or more rows in the table (columns from your CSV file) to use the "Batch edit options" button to apply the column options described above to multiple CSV columns at once. The options include multivalue separator, language, data type, and property privacy.
...
These settings apply to the entire CSV you are importing. Note these settings can be overwritten by column options in the Map to Omeka S data tab. Mapping a column for template, class, or owner, those values will override these settings. So will column settings for language and privacy.
...
!!! note
	You might upload different formats of data (for example, some text-based creator names and some URI-based creator links) into the same field (`dcterms:creator`, in this case). If so, use two columns (named something helpful like "dcterms:creator-text" and "dcterms:creator-uri"). Upon import, map those two columns to different data types. Use the wrench icon to open up column mappings and select the correct data type for each column.
...
- **Action on unidentified resources**: This option determines what to do when no matching resource exists in the Omeka S installation when your selected action applies to an existing resource. For example, "Append", "Revise", "Update", or "Replace". This option is not used when the main action is "Create" or "Delete". Your options are:
	- Skip the row and ignore its contents
	- Create a new resource with the information supplied.

In addition to the above, the Advanced Settings tab has an option to set the number of rows to process by batch. By default this is set to 20. If you are running into errors with an import, you may want to set it to 5 or even 1 to troubleshoot. This will help determine the source of the error.
...
!!! note
	...
	Revising, Updating, and Replacing data will **erase** data supplied in earlier rows of your CSV if later rows use the same identifier. To import multiple values (for example, two Creator values) in these processes, you can either: put them in two columns in the same row, mapped to the same property. Or, put them in a single cell and use multivalue separators. Do not forget to specify your multivalue separator in the "Basic Settings" tab. Also check the "Use multivalue separator" box in the options (wrench icon) for each column.
...
**Properties**: Select a property to map the column data to. This can be from any of the installed vocabularies. Use the Filter field to search the available properties for a specific property.

![Properties option open showing all installed vocabularies for the Omeka S installation: Dublin Core, Bibliographic Ontology, Friend of a Friend, Scripto and OWL-Time Ontology.](../modules/modulesfiles/csvimport_itemsMapProp.png)
...
You may have data in a column in your CSV which you do not want to bring in to your Omeka S installation. If so, simply do not map that column to a property or data type.
...
To access options for data in a column of your CSV click the wrench icon for that column heading. This is represented by a row in the import table.

This will open a drawer on the right side of the browser window with the following options:

- **Use multivalve separator**: Check this box to use the multivalue separator for data in this column. You set the multivalue separator in the initial import page. You can change it later in the Basic Settings tab.  
- **Language**: Set the language for this column using the [IETF Language tag](https://en.wikipedia.org/wiki/IETF_language_tag){target=_blank} for the language in which the text is written. This will override what you have entered in basic settings.
- **Import values as private**: Check this box to set all property values *in this column* private.
- **Data type**: A dropdown with at least three options. These options correspond to the [values](../content/items.md#values) one can use when adding properties to an item:
	- Import as text (default).
	...
	- Import as Omeka S resource. This will create [linked resources](../content/item-sets.md#linked-resources). If you select this option, you must choose which property values to match to find the intended Omeka resource in your installation. Do this in the next **Resource identifier property** dropdown. This must be a unique property, so "Title" may not be a good choice. 
		- You can use the internal Omeka ID. A resource's ID is the number sequence at the end of the URL when on the view or edit page. For `/admin/item/11576`, the ID is 11576. You can also see the resource's ID in the right-hand drawer on the resource's view page. Items, item sets, and media all have IDs.
		- You can include resources that are being made in the same CSV. You can do this as long as the resources being linked to have already been created in earlier rows and can be found with the unique property value indicated here. To do this, we recommend setting the batch number low (even to 1) on the Advanced Settings tab. This ensures resources are being fully created before another new resource tries to link to them. 
	- Certain modules, such as Numeric Data Types, may supply additional data type options when installed.

Be sure to click the "Apply changes" button at the bottom of the drawer in order to save your changes. To remove a column option setting, click the wrench icon again and undo your changes manually.

#### Batch edit
When you select one or more rows in the table (columns from your CSV file), use the "Batch edit options" button to apply the column options described above to multiple CSV columns at once.
...
- **Open/closed to additions**: Set whether users other than the owner  will be able to add or edit the item sets. This also applies to site and global admins.
- **Multivalue separator**: Enter the multivalue separator character here, if you have used one.
...
The Advanced Settings tab has an option to set the number of rows to process by batch. By default this is set to 20. If you are running into errors with an import, you may want to set it to 5 or even 1 in order to troubleshoot. This helps determine the source of the error.
...
- **Create a new resource**: Default option. Each row in the CSV will become a new resource.
- **Append data to the resource**: Add new data to the resource, based on an identifier for an existing resource. This annot be undone. This option allows you to supply multiple values for the same item set. Each row will be appended. That is, you can append one title to an item set in one row, and append another title to the same item set in another row.
- **Revise data of the resource**: Replace existing data of the resource with data from the CSV, *except if* the corresponding cell in the CSV is empty. This cannot be undone.
- **Update data of the resource**: Replace existing data of the resource with data from the CSV, *even when* the corresponding cell in the CSV is empty. This cannot be undone.
- **Replace all data of the resource**: Remove all properties of the resource, and fill with new information from the sheet. (Cannot be undone.)
- **Delete the resource**: Delete all matching resources. (Cannot be undone.)
...
- **Action on unidentified resources**: This option determines what to do when no matching resource exists in the Omeka S installation. The selected action only applies to an existing resource ("Append", "Revise", "Update", or "Replace"). This option is not used when the main action is "Create" or "Delete". Your options are two radio buttons:
	- Skip the row
...
!!! note
	...
	Revising, Updating, and Replacing data will **erase** data that was supplied in earlier rows of your CSV if later rows use the same identifier. If you wish to import multiple values (for example, two Creator values) in these processes, you can either put them in two columns in the same row mapped to the same propert or put them in a single cell and use multivalue separators. Do not forget to specify your multivalue separator in the "Basic Settings" tab. Also check the "Use multivalue separator" box in the options (wrench icon) for each column.

### Complete import
After completing mappings, column options, and any settings, click the "Import" button in the upper right corner of the browser window. This should start the import and redirect you to the Past Imports tab. You should see a confirmation message saying "Importing in Job ID [number]".

## Import media
To import media, select "Media" under the "Import type" on the first page.

In order to import media, you must have a column in the CSV that maps to item data. Media only exists when associated with an item by a unique identifier. This unique identifier might be the Omeka item ID, title, etc. If you are running an advanced job, such as an update or replace, then you do not have to have a column with associated item data. You only need a way to match the rows to media that already exist.
...
You can access options for data in a column of your CSV. This is represented by a row in the import table. To do so, click the wrench icon for that column heading. 
...
- **Data type**: A dropdown with at least three options. These options correspond to the [values](../content/items.md#values) one can use when adding properties to an item:
	...
	- Import as Omeka S resource. This will create [linked resources](../content/media.md#omeka-resource). If you select this option, you must choose which property values to match to find the intended Omeka resource in your installation. Do this in the next **Resource identifier property** dropdown. This must be a unique property, so "Title" may not be a good choice. 
		- You can use the internal Omeka ID. A resource's ID is the number sequence at the end of the URL when on the view or edit page. For `/admin/item/11576` the ID is 11576. You can also see the resource's ID in the right-hand drawer on the resource's view page. Items, item sets, and media all have IDs.
		- You can include resources (items or item sets) that are being made in the same CSV. You can do this as long as the resources being linked to have already been created in earlier rows and can be found with the unique property value indicated here. If you wish to do this, we recommend setting the batch number low (even to 1) on the Advanced Settings tab. This ensures resources are being fully created before another new resource tries to link to them. 
...
#### Batch edit
You can use the "Batch edit options" button to apply the column options described above to multiple CSV columns at once. To do this, select one or more rows in the table representing the columns from your CSV file.
...
These settings apply to the entire CSV you are importing. Note these settings can be overwritten by column options in the Map to Omeka S data tab. If a column is mapped for template, class, or owner, those values will override these settings. This is also true for column settings for language and privacy.
...
- **Update data of the resource**: Replace existing data in the resource with data from the CSV. This applies even when the cell is empty.
...
In addition to the above, the Advanced Settings tab has an option to set the number of rows to process by batch. By default, this is set to 20. However, if you are running into errors with an import you may want to set it to 5 or even 1 in order to troubleshoo. This will help determine the source of the error.
...
**Item-specific data** has a dropdown to set Item Set by selected property. You may have a column with data for an Item Set to which you want to add the item. Use this dropdown to set how it maps. You can either use the Item Set's internal ID, or any one of its properties (title, description).
...
**Media-specific data** has a dropdown to set the item to which the media should be added. You can either use the item's internal ID, or any one of its properties (title, description). An item's ID is the number sequence at the end of the URL when on the view or edit page. For `/admin/item/11576` the ID is 11576. You can also see the item's ID in the right-hand drawer on the item's view page.
...
You may have data in a column in your CSV that you do not want to bring in to your Omeka S installation. If so, simply do not map that column to a property or data type.
...
- **Use multivalue separator**: Check this box to use the multivalue separator for data in this column. You set the multivalue separator character in the initial import page. You can change it later in the Basic Settings tab.
- **Language**: Set the language for this column using the [IETF Language tag](https://en.wikipedia.org/wiki/IETF_language_tag){target=_blank} for the language in which the text is written. This will override what you have entered in basic settings.
- **Import values as private**: Check this box to set all property values *in this column* private.
- **Data type**: A dropdown with at least three options. These options correspond to the [values](../content/items.md#values) one can use when adding properties to an item:
	- Import as text (default).
...
- **Owner**: Set the owner for the item sets by selecting a user from the drop-down menu. You can use the search field at the top of the dropdown to narrow results or find a particular user.
- **Visibility**: Set the visibility of the imported item sets as public  or private.
- **Item sets open/closed to additions**: Set whether users other than the owner will be able to add or edit the item sets. This also applies to site and global admins.
...
- **Resource identifier property**: Select from a dropdown of all properties in your Omeka S installation. This should be the property in which you already have data, that you used to create the column data above.
	...
	- This will only work with exact matches.
	- If you have more than one resource with matching data, it will only take action on the oldest resource.
- **Action on unidentified resources**: This option determines what to do when no matching resource exists in the Omeka S installation. This action only applies to an existing resource ("Append", "Revise", "Update", or "Replace"). This option is not used when the main action is "Create" or "Delete" Your options are two radio buttons:
	- Skip the row
	- Create a new resource.

In addition to the above, the Advanced Settings tab has an option to set the number of rows to process by batch. By default this is set to 20. However, if you are running into errors with an import you may want to set it to 5 or even 1 in order to troubleshoot. This helps determine the source of the error.
...
## Import users
When importing users, you can only bring in data related to the user table in Omeka S. This includes the user's email, display name, and role. Any additional data in your CSV will not be imported. None of these can be multivalue properties.
...
!!! note
	Users will **not** be notified via email when an account is created for them using CSV Import. When you create an account manually, the email address receives a notification with a link to set their password. Currently, CSV Import does not send those emails so users cannot set their own passwords. Site administrators must set passwords and then share them manually with users created via CSV Import.
...
Depending on the size of the import, it may take some time to undo. On complete, the status of the import on the "Past Imports" table will say "Undone." This is followed by the date the import was reversed.
...
### Custom Vocab
If you have [Custom Vocab](customvocab.md) installed and active, it will add your custom vocabularies as data types. You can select these data types during your CSV Import. 

### Extract Text

When enabled, the [Extract Text module](extracttext.md) attempts text extraction on any files ingested through a CSV Import process. If available, this will include Tesseract, an image processor that employs optical character recognition (OCR). You can disable individual extractors in the Configuration menu of Extract Text. This prevents them from running during your import. 

!!! note
	We recommend you disable Tesseract, or the entire Extract Text module, when performing an import including images. OCR running on images without text in them is processor-heavy and may cause problems with your import. 
...
For the data in this column, you need to include the full file name, including extension. For example, if you want to import a JPG file named "Jekyll_and_Hyde_Title", then the data in the media column of the CSV you are importing should be `Jekyll_and_Hyde_Title.jpg`.
...
Be sure to format data precisely When importing it as numeric. Always check to be sure your data has imported correctly into the format of your choice. CSV Import will leave values empty if the formatting isn't recognized. 
...
Use the following ISO 8601 formats for importing [durations](https://en.wikipedia.org/wiki/ISO_8601#Durations){target=_blank}. This is expressed as number of years, number of months, number of days, etc.:
