...
The [Persistent Identifiers module](https://omeka.org/s/modules/PersistentIdentifiers/){target=_blank} lets users to create or import persistent identifiers (PIDs). They can then assign these PIDs to Omeka S items. Users can assign PIDs at item creation, via the item edit screen, or by batch edit. Remove them using the item edit or batch edit screens. PIDs may be "extracted" from existing metadata fields. Once assigned, accessing an item's PID in-browser resolves to a stable, non-site-specific landing page. This contains the item's metadata, media, and any sites to which the item is assigned.
...
+ **Assign PID to New Items**: Check this box to automatically newly mint or extract an existing PID and assign to every new item in Omeka S. It will do this whether it is created in Omeka S or imported.
+ **Fields with Existing PIDs**: A list of metadata fields separated by commas. They may contain PID values within a newly created or imported item. If found, the existing PID will be assigned to the item instead of a new PID.
...
+ **NAAN & Shoulder Namespace**: The Name Assigning Authority Number (NAAN) and ARK shoulder value uniquely assigned to an organization. This will appear in every ARK generated. 
+ **EZID Username**: The EZID user with permission to create and update identifiers for the above namespace. 
+ **EZID Password**: The password for the above EZID user. For security reasons this password doesn't persist in the form. If changes are made to the **NAAN & Shoulder Namespace** or **EZID Username** you must also re-enter the password before pressing "submit".
...
+ **DataCite Password**: The password associated with the above **DataCite Repository ID**. For security reasons, this password doesn't persist in the form. If changes are made to any field on the DataCite Configuration screen, you must also re-enter the password before pressing "submit".
...
DataCite requires five descriptive metadata values in order to generate a DOI. These values are Title, Creators, Publisher, Publication Year, and Resource Type. All of these fields must be mapped to an existing metadata field. Select theat field from the list of available vocabularies in your Omeka S instance. The Resource Type General property must exactly conform to a [ResourceTypeGeneral controlled vocabulary](https://support.datacite.org/docs/datacite-metadata-schema-v44-mandatory-properties#101-resourcetypegeneral){target=_blank} value.
...
If you select "Remove PID," a drawer will open on the right warning. It warns you that this will remove the PID and break any incoming links. Click "Confirm Remove PID" to delete the PID.
...
![Batch edit items screen, with fields from a number of modules. Near the bottom is a "Persistent Identifiers" entry with three radio button options. They are "Mint PIDs", "Remove PIDs", and "[No action]". A blue arrow points to this entry.](../modules/modulesfiles/PID_batchoptions.png)
