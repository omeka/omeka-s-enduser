...
The [Faceted Browse module](https://omeka.org/s/modules/FacetedBrowse){target=_blank} lets you create "browse resource" pages with facets. These Facets are filter and sort functions. Site visitors can use them to explore your collections. Faceted Browse pages can be made for items, items sets, or media. 

With this module, site managers are able to configure faceted browse pages. They can also add them to their site's [navigation](../sites/site_navigation.md). End users are then able to browse through resources and use the facets to narrow the results. This functionality is similar to the filtering options on many websites. It should be easy for users to manage as long as you have clear language.

The [public view](#public-views) section below shows how these facets display for single and multi-category pages.

Faceted browse pages exist as separate [pages](../sites/site_pages.md). You can [add faceted browsing to a page as a block](#faceted-browse-preview-page-block) that will preview the FB page. However, it won't have its full functionality. 
...
- Facet: an aspect of a resource, usually part of the metadata. It becomes a way to filter the resources in the category. Faceted browsing works best when you have some controlled vocabularies in your metadata values. Or, when unique values can be sorted into buckets (such as dates browsed by century). 
...
- Select: When a facet is browseable, allow your visitors the most appropriate means of sorting or searching those facets. For example, allow them to exclude or include strings in the results or match full values exactly. Or, with numerical data types, bucket the values into groupings that you set. 
...
Use the **resource type** dropdown to select the resource type that you would like to let users browse on this page. This includes Items, Item Sets, or Media. You cannot edit this after page creation.
...
Once the page has been created, you must create a Category. This is where facets are created. Use filters to narrow down the pool of resources to browse with those facets. You can use multiple categories to provide different resource subsets for users to start their browsing.

For example, you may want to add a general "Browse items" page to your navigation. It can have a category for navigating only images, another category for navigating only events, and a final category that allows users to browse all items in the site=. Or you could create separate Faceted Browse pages to add to your navigation. You could make one for each item class. Each page can provide categories to restrict the resources further by another value. 

Click the "Add category" button to be taken to a new interface. 

Give your category a **name** to be displayed to the public. This will appear at the top of the Faceted Browse page if it is the only category. It will appear in the selection menu on the page if there are multiple categories. This is the only required field in this section. 

Use the **search query** interface to set the pool of resources that users will browse. The "Edit" button opens a drawer on the right hand side of the browser window. It works exactly like [advanced search forms](../search.md#item-advanced-search) for items, media, and item sets. The "Advanced edit" button allows you to input a query string. You can leave the query blank to include all of the site's resources of that type. 

You can set a **default sorting** method to use when a site visitor starts to browse using this category. This will apply to the default list of resources, or to the [FB table you have customized with columns](#columns). The dropdown menu will show "Created" and "Title" at first. But, it will update to reflect the columns you customize further down on the page. You will need to save your columns settings and return to this page to see this menu updated. Note that you cannot set a default sort by an Item Set column. 
...
For the Single (list) and Multiple (list) Select types, page creators can choose to truncate the values on the list that is visible to the site visitor. They can do this by setting a number in the "Truncate values" option. Leaving the input blank will display all values. Entering in a number only displays that number of facets in order. A "See more (X)" link displays the number of further facets. 
...
!!! note
	The "Value facet mode" setting at the top of the category editing interface will apply to all value facets you add to this category. "Match any" will allow users to broaden their results by adding more selections. "Match all" will narrow the results when adding more criteria.

Use the dropdown to select which property to use for the facet. For example, you may wish to select your description field, and allow users to search within those description texts. Or you may wish to select the subject field. This allows users to see all the controlled values you are using as subject headings. You can leave this blank to provide searching or browsing options across all resources' properties.

Set the Select type for the browse faceting. This sets how site visitors interact with the options in the field:

- Single (list). Visitors can select only one. All options are display in a list of radio buttons.
- Multiple (list). Visitors can select multiple. All options display in a list of checkboxes. Multiple selections by site visitors will narrow (Option 1 AND Option 2) the search results. 
...
- "Is exactly": Visitors pick a value that is an exact match to the property value. 
	- For example, visitors can check a box next to an available subject value to see all items with that exact value in their subject property. Exact text is not case-sensitive.
- "Is not exactly": Visitors pick a value that exactly matches items to exclude from the results. 
	- For example, visitors can check a box next to an available subject value to see all items that do not have that exact value in their subject property.
...
You cannot leave the query blank. You may wish to provide multiple types of Select queries for the same field. This offers more granularity to site visitors.

For the Single (list) and Multiple (list) Select types, Page creators can choose to truncate the values available on this list that is visible to the site visitor. Do this by setting a number in the "Truncate values" option. Leaving the input blank will display all values. Entering in a number will display only that number of facets, in order. A "See more (X)" link displays the number of further facets.
...
The order of your available values (from most to least frequent) will not persist when you use the "Add all" button. Properties will rearrange themselves according to an internal order. So will resource templates, classes, and resources by ID. 
...
Once satisfied with your settings, click the "Set facet" button.

For example, you may wish to load all the "Subject" property's values. This allows people to browse items using the subject headings currently in use. Select "Show all available values" to see a list of subjects currently in use from most frequent to least. To make faceted browsing more useful, clean up your data and consolidate similar values or fix typos and variations. You can use the [Value Suggest module](../modules/valuesuggest.md) in tandem with Faceted Browse to view and clean messy data.

!!! note
	"All available values" facets do not dynamically update when adding new values to the corpus or editing existing values. You must reload the options using "Show all available values" and "Add all" in the facet. This updates the browsing list's content. We recommend doing this regularly when adding new items.
...
If you are using the [Numeric Data Types module](../modules/numericdatatypes.md), you will have additional facet types to work with. This includes Date after, Date before, Value greater than, Value less than, Duration greater than, Duration less than, and Date in interval.
...
Once you have selected a facet type, you can configure the facet to operate with the properties that use a numeric data type. Only properties with the exact data type set (Number, Date, Duration, or Interval) will display in the dropdown.

In the public view, faceting is controlled through a dropdown menu.
...
The items on the public page will initially display in a list. This includes the title, description, and thumbnail for each resource. These rows may be truncated (overly long text will be hidden).

You can configure the information displayed about the results. Do this by adding columns of metadata to the display for your faceted browse. This will convert the display into a table with a row for each resource in the results. Columns are set on a category-by-category basis. The display of the initial page may change depending on whether your page has one category or multiple categories.

On the public view of a faceted browse, users can sort by a column by selecting it from the dropdown menu. You can sort each column in ascending or descending order. To prevent users from sorting by a certain column, you can check the "Exclude sort by" checkbox when configuring that column to exclude it from the dropdown menu.

!!! note
	If a value is very long, such as the resource title or description, you may end up with very tall table rows. The largest field will occupy the widest column and other columns will narrow to compensate. You can use the [CSS Editor module](csseditor.md) to implement a truncation where overflowing text is hidden.
...
Then, set the maximum number of values for that property. To display all values, leave the input blank. This may cause very tall rows in your table if items have multiple values for the selected property. This may also occur if some value are very long. 
...
The public views of Faceted Browse pages include the base page. This page may include one or more categories to click on. Public views also include category-specific pages, featuring the columns chosen for that view. Browse facets will show in a list if no columns are selected for display. It won't include column headings, but the items on the page can be sorted as usual.
...
In this image, the facet is Era, displayed as a single-choice list. The items for this page display in columns with the title and era for each item.

You may have more columns than the user's browser window can accommodate. This depends on your columns settings on each category. If this is the case, they will see only the number of columns that can fit their contents on the page without overflow. They will see left and right arrows above the top-right corner of the table to scroll the hidden columns into view. Text in columns that are not showing on the page will not come up if the user text-searches the page.
...
When there are multiple categories on a page, it will load with all resources from all categories displayed. It will show links to the categories available in a submenu.
...
Once a user clicks on a category, the resource list will change. It will display only that category's resources, the column display you have set, and its facets in the submenu. Users can use a "Back" button on the page to return to the full list of categories and clear their filters.
...
You can add a "Faceted browse preview" page block to pages in Omeka sites. This page block will show the first few items that appear on the full FB page. It won't include any of the sidebar facets or navigation tools available on the page. 

![Faceted browse preview page block in the admin settings, with settings as listed below. A page category is selected. Limit is set to 6. Preview title is set to "Browse by type". Link text is set to "Browse all".](modulesfiles/FacetedBrowse_previewpageblock.png)
...
* Preview title: Enter a title to display at the top of the page block. 
* Link text: Enter text to display on a button at the bottom of the page block. This will link the users to the full FB page. 
...
You can channel your users to a Faceted Browse page directly from the search bar in your site's header or menu. 

Go to your site's Settings page, and look for the "Faceted Browse" section. You will see a dropdown menu. This allows you to select one of your categories to display when performing a text search in the site-wide search bar. 
...
When a user searches your site by text, they will be taken to the Faceted Browse category. Their text search will fill out in the full-text facet in the sidebar. They will be able to further narrow or broaden their searches from there. 
