...
When an "Import" button is clicked for any of the datasets, an indicator will appear to show that the dataset is importing. You can then click on the "Job" or "Log" links to keep track of the background process. Each import should only take a minute or two. You can refresh this page by clicking again on the "Demo Data" link in the sidebar. 

![A dataset being imported. A green bar across the top of the screen indicates the import in process. The dataset box now shows "Importing..." besides the data title, and links for "Job" and "Log".](modulesfiles/demodata_import.png)

When the import is finished, the dataset will show "Imported" in green beside the data title. Links are now available to see the Items and Item sets created for this dataset. 

!!! note
	Imported resources will not be added to any sites. Users can manually add the items to a site to view their display on the public side of Omeka. We strongly recommend you only add these resources to a private site, to avoid mistaken indexing of this content by search engines.

You will now have "Re-import" or "Purge" buttons available for each imported dataset. Purging a single dataset will remove the related resources from your installation, but the module is still occupying storage space on your server. Purging will remove the related resource template of each dataset as well as all its resources. 

![The Demo Data screen showing a dataset being purged.](modulesfiles/demodata_purge.png)

### Uninstalling

The module requires storage space of more than 300 megabytes. This is by far the largest Omeka Team module. It is intended to provide Omeka S users with sample items for learning various features. When you are done with it, it should be both uninstalled in the interface and manually deleted from the `/modules` folder of your server to free up storage space. 
