...
Each entry in the list of values will have a separate colour. The graph will display instances of each value in each date bucket (e.g. each year, or each ten-year span). When a date bucket is hovered over with the mouse, a box will pop up showing precise counts of each value, in the order provided in the settings.
...
![Admin-side settings of the above diagram.](../modules/modulesfiles/dataviz_countPropertyValuesTime1.png)

![More admin-side settings of the above diagram.](../modules/modulesfiles/dataviz_countPropertyValuesTime2.png)
