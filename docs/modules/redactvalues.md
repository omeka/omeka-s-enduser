...
Logged-in users at the Editor, Supervisor, or Global Admin levels can always see what is redacted. Your installation admin can set viewability for users at the Author or Researcher level.
...
- build a query to filter the resources
- select the property from which to redact text
- select the pattern to use for redaction
...
You can also add a query to filter the resources from which to redact text. You can add a query via the "Edit" button. This will open a drawer on the right that provides options for narrowing your search. Or, you can use "Advanced Edit" to add your query directly from the search results page. Copy and paste everything in your browser's address bar, starting with the question mark to the end of the search URL (to the right). 

In either circumstance, be sure to click the "Apply" button to add your query to the form. No query means you redication will include all resources of the specified resource type. 

![Edit query tray in add Redact Values form.](../modules/modulesfiles/redactValues_query.png)

There is a field to include the replacement text that will appear in place of the redacted values. If you leave this blank, it will hide the entire redacted field.

Each redaction has individual viewing options. You have the option to give certain role types permissions in viewing redactions. Any user with permission to update a resource can automatically view its redacted text. You can allow authors or researchers by checking their respective boxes. Normally, values will be redacted for authors except for resources that they own. Values will be redacted for researchers unless you grant them permissions in individual redactions.
...
You can also sort your existing redactions by date or label. Select ascending or descending order via the "Sort" function in the upper right corner.

![Sort function for list of existing redactions.](../modules/modulesfiles/redactValues_sort.png)

## Add patterns

You can customize the patterns available for redactions. Select "Patterns" under Redact Values in the Modules tab of the left-hand navigation. To create a new pattern, select the "Add Pattern" button in the upper right corner. You will be asked to provide both a **Label** and a **Pattern**.
...
Once you have added this information, click "Submit" in the upper right corner. Your added pattern should appear in the Pattern dropdown menu when adding a redaction.
